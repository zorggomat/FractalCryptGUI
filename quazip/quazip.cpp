...
bool QuaZip::open(Mode mode, zlib_filefunc_def* ioApi, unsigned long long eocdPos, int posDefined)
{
  p->zipError=UNZ_OK;
  if(isOpen()) {
  ...
  switch(mode) {
    case mdUnzip:
      if (ioApi == NULL) {
          ...
          p->unzFile_f=unzOpenInternal(ioDevice, NULL, 1, flags, eocdPos, posDefined);
      } else {
